...
public interface UserMapper {
	...
	@Select({"<script>","select * from useraccount ","<if test='userid == null'> where userid != 'admin' </if>","<if test='userids != null'> and userid in <foreach collection='userids' item='id' open='(' close=')' separator=','>#{id}</foreach> </if>","<if test='userid != null'> where userid = #{userid} </if>","</script>"})
